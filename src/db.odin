...
// ensure_schema initializes the database schema if it doesn't exist.
// Creates the cmd_history table with columns for storing command history.
ensure_schema :: proc(db: ^sql.DB) -> sql.Error {
	// Create the cmd_history table if it doesn't exist
	schema := `
	...
	if exec_err != nil {
		log.errorf("unable to execute schema creation: %s", exec_err)
		return exec_err
	}
